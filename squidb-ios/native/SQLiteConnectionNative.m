...
+ (NSObject *) nativePrepareStatement:(NSObject *)connectionPtr withSql:(NSString *)sqlString {
    ...
    IOSByteArray *sql = [sqlString java_getBytesWithCharsetName: @"UTF-16"];

    // change IOSByteArray *sql = [sqlString getBytesWithEncoding:NSUTF16StringEncoding];
...
+ (void) nativeBindString:(NSObject *)connectionPtr statement:(NSObject *)statementPtr index:(jint)index value:(NSString *)value {
    ...
    //change const IOSByteArray *bytes = [value getBytesWithEncoding:NSUTF16StringEncoding];
    const IOSByteArray *bytes = [value java_getBytesWithCharsetName: @"UTF-16"];
...
+ (NSString *) nativeExecuteForString:(NSObject *)connectionPtr statement:(NSObject *)statementPtr {
    ...
    if (err == SQLITE_ROW && sqlite3_column_count(statement.statement) >= 1) {
        ...
        if (text) {
            ...
            return [NSString java_stringWithCharacters:chars];
            // change return [NSString stringWithCharacters:chars];
        }
    }
    return nil;
}
